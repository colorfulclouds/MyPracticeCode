...
public class borrow_operate extends JPanel{
	...
	JLabel prompt = new JLabel(new ImageIcon("borrow.png"));//提示新的图书
	...
	String[] title = {"编号","读者唯一编号","ISBN","借书时间","应还时间","是否归还"};
	...
	public borrow_operate(String id, String psd) {
		...
		//放置添加的信息的容器
		add_panel.setLayout(new GridLayout(2,5,5,15));//编号不用添加
		...
		table = new JTable(dtl){
			...
			public boolean isCellEditable(int rowindex,int colindex){
			    if (colindex == 0 || colindex == 3 || colindex == 4 || colindex == 6) return false; //这里需要新加入===========
			    return true;
			 } 
		};//存放信息的表格
...
//		table.getTableHeader().setResizingAllowed(false);   //不可拉动表格宽度不可以变化
		...
		//一开始 显示出来
		{
			...
			sql = "select * from LMS_view_borrow_booktype ";//查询语句
			try {
...
//				table.setCellEditor(new DefaultCellEditor(type));
		...
		jrb[3].addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(dtl.getColumnCount() <= 6)//在超过范围后就不在出现了//====
					dtl.addColumn("已选中");//======
				//恢复点走的时候的状态
...
//							e1.printStackTrace();
				...
				if(flag == 2){
					...
					try {
						...
						for(int i = 0;i<5;i++)//增加完一个后 清空图书信息文本框
...
//							e1.printStackTrace();
					...
					sql = "select * from LMS_borrow";//查询语句
					try {
						rs = stm.executeQuery(sql);
						while(rs.next()){
							Vector v = new Vector();
